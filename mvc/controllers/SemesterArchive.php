...
class SemesterArchive extends CI_Controller {
  ...
  public function semester_archive(){
    $data= [];
    $data['title'] = "Semester Archive";
    $data['semester_archive']='semesterArchive/view';
    // get department data
    $data['get_all_data'] = $this->semesterArchive_m->get_assign_info();
    $this->load->view('dashboard_layout',$data);
  }

  // view details accroding to subject
  public function viewdetails($id = null){
    if($this->semesterArchive_m->assign_teacher_details_info($id)) {
      $get_student_mark_list=$this->semesterArchive_m->assign_teacher_details_info($id);
      $data= [];
      $data['title'] = "View Details";
      $data['get_list'] = $this->semesterArchive_m->get_student_list($get_student_mark_list->semester_code,$get_student_mark_list->sub_code,$get_student_mark_list->std_batch);
      $data['gradePoints'] = $this->gread_point_m->get_gread_point();
      // dd($data['gradePoints']);
      $data['details'] = $this->semesterArchive_m->assign_teacher_details_info($id);
      $data['assign_teacher_details']='semesterArchive/details';
      $this->load->view('dashboard_layout',$data);
    }else {
  ...
  public function updateResult(){
    ...
    $this->semesterArchive_m->get_marks($attr,$this->input->post('get_id'));
